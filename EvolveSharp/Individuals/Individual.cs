...
{
    private readonly IList<double> _genes;
    ...
    public Individual(IList<double> genes)
    {
        _genes = genes;
    }
    ...
    public Individual(Individual genome)
    {
    ...
    public override bool Equals(object obj)
    {
        ...
        var genome = obj as Individual;
        if (genome == null)
            return false;

        for (int locus = 0; locus != _genes.Count; locus++)
            if (_genes[locus] != genome._genes[locus])
                return false;
    ...
    public object Clone()
    {
        var newGenes = new double[_genes.Count];
        for (var i = 0; i < _genes.Count; i++)
        {
            newGenes[i] = _genes[i];
        }
        return new Individual(_genes) {_fitnessFunction = _fitnessFunction};
    }
}
}
